...
bool DibSect::CopyClient(HWND hwnd, int left, int top, int right, int bottom)
{
    RECT r;
    r.left = left;
    r.right = right;
    r.top = top;
    r.bottom = bottom;
    return this->CopyWndClient(hwnd, &r) != 0;
}

bool DibSect::CopyWindow(HWND hwnd)
{
    RECT r;
    GetWindowRect(hwnd, &r);
    return this->CopyWndClient(GetDesktopWindow(), &r) != 0;
}
